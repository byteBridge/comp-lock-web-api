...
module.exports = class MockBillingServer {
  ...
  async initiateSubscription({
    ...
    const response = await axios.post(
      "https://paynow.now.sh/pay/mobile",
      {
        products,
        authemail,
        mobileNumber,
  ...
  async pingSubscription(reference) {
    return (await axios.get(
      "https://paynow.now.sh/complock/ping-subscription/" + reference
    )).data;
  }
  ...
  async getComputerTokens(reference) {
    const response = (await axios.get('https://paynow.now.sh/complock/tokens/' + reference)).data;
    return response;
  }
